...
def generate_straight_trajectory(x_start, x_end, t, total_time, time_step):
    speed = (x_end - x_start) / (total_time)  # Assuming total time of 300 units for trajectory
    x_d = x_start + speed * time_step * t  # x position changes linearly with time
    y_d = 5  # y is constant
...
for i in range((int(total_time/time_step))):
    x_d, y_d, x_dp, y_dp, x_dpp, y_dpp = generate_straight_trajectory(0, 10, i, total_time, time_step)
    v_c, w_c = control_law(x, y, theta, x_d, y_d, x_dp, y_dp, x_dpp, y_dpp, K1=10.0, K2=10.0, K3=5.0)
