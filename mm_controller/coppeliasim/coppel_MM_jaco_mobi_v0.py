...
try:
    ...
    while (t := sim.getSimulationTime()) < total_time:
        ...
        manipulability_2D_direction = manipulability_direction[:2]
        ...
        # 벡터 간 각도 계산 (반시계방향 회전)
        cross_2D = np.cross(manipulability_2D_direction, d_goal_unit_2D)
        dot_2D = np.dot(manipulability_2D_direction, d_goal_unit_2D)
