...
while simulation_app.is_running():
    ...
    if world.is_playing():
        ...
        if  not reached_default:
            ...
            # print("current_joint_positions shape:", current_joint_positions.shape)
            ...
            # print("diff:", np.abs(current_joint_positions[4:10] - target_joint_positions[4:10]))
            # 목표 자세 도달 여부 체크
            ...
            # euler_zyx = r_ur.as_euler('zyx', degrees=True) # zyx 순서로 euler angles 추출
            # print('euler_zyx:', euler_zyx)  
            # print('quat norm:', np.linalg.norm(quat_xyzw))
            ...
            # H = np.zeros((n_dof, 6, n_dof))  # same as jacobm

            # for j in range(n_dof):
            #     for i in range(j, n_dof):
            ...
            # JJ_inv = np.linalg.pinv((J_mb @ J_mb.T)).reshape(-1, order='F')

            # # Compute manipulability Jacobian
            # J_m_T = np.zeros((n_dof, 1))
            ...
            # epsilon = np.zeros((n_dof,1))
            # epsilon[0] = - k_e * θε  # 베이스 x 위치 오차
            ...
            # wc = -float(a_v)
            vc = 0.0
            wc = 0.8  # 작은 속도에는 회전을 하지 않음. 또한 회전 시에 제자리에서 회전하지 않음.
            r = 0.165
            l = 0.582
            w_R = vc/r + l*wc/(2*r)
            w_L = vc/r - l*wc/(2*r)
            ...
            joint_velocities[3] = 0.0 #w_R
            joint_velocities[4] = 0.03
            joint_velocities[5] = 0.0   
            joint_velocities[6] = 0.03
            joint_velocities[7] = 0.01
            joint_velocities[8] = 0.0
            joint_velocities[9] = 0.0
