...
class World_Robot(Node):
    def __init__(self):
        ...
        # ✅ 새로운 데이터 구조
        self.trajectory_data = []  # 각 위치별 상세 데이터
        self.collecting_data = False  # 데이터 수집 중 플래그
    ...
    def collect_data_sequence(self):
        ...
        self.get_logger().info("📊 데이터 수집 시작... (5개 샘플, 0.5초 간격)")
    ...
    def analyze_and_store_samples(self, samples):
        ...
        for cam_name, stats in camera_stats.items():
            self.get_logger().info(f"   {cam_name}: {stats['sample_count']}샘플, 마커평균={stats['avg_markers']:.1f}개")
    ...
    def display_images(self, img_1, img_2_world, img_2_robot, img_3, 
        ...
        if robot_result_cam1 is not None:
            ...
            if 'cam1' in H_world_to_robot_results:
                ...
                cv2.putText(display_img_1, f"Pos: [{pos[0]:.3f}, {pos[1]:.3f}]", 
        ...
        if self.collecting_data:
            ...
            cv2.putText(display_img_2_world, "Press SPACE to collect 5 samples", 
        ...
        if robot_result_cam2 is not None:
            ...
            if 'cam2' in H_world_to_robot_results:
                ...
                cv2.putText(display_img_2_robot, f"Pos: [{pos[0]:.3f}, {pos[1]:.3f}]", 
                        (10, 60), font, 0.5, (0, 255, 0), 2)
        ...
        if robot_result_cam3 is not None:
            ...
            if 'cam3' in H_world_to_robot_results:
                ...
                cv2.putText(display_img_3, f"Pos: [{pos[0]:.3f}, {pos[1]:.3f}]", 
                        (10, 60), font, 0.5, (0, 255, 0), 2)
    ...
    def plot_trajectory(self, show_only=False):
        ...
        if not show_only:
            ...
            filename = f'robot_trajectory_detailed_{timestamp}.png'
            plt.savefig(filename, dpi=300, bbox_inches='tight')
            self.get_logger().info(f"상세 Trajectory 저장됨: {filename}")
            
            # 상세 데이터 CSV 저장
            self.save_detailed_trajectory_data(timestamp)
        
        plt.show()
    
    def save_detailed_trajectory_data(self, timestamp):
        """상세 trajectory 데이터를 CSV로 저장"""
        try:
            import pandas as pd
            ...
            for i, data in enumerate(self.trajectory_data):
                main_row = {
                    ...
                    'overall_x_mean': data['overall_mean']['x'],
                    'overall_y_mean': data['overall_mean']['y'],
                    'overall_x_std': data['overall_mean']['x_std'],
                    'overall_y_std': data['overall_mean']['y_std'],
                ...
                for cam_name in ['cam1', 'cam2', 'cam3']:
                    if cam_name in data['camera_stats']:
                        stats = data['camera_stats'][cam_name]
                        main_row.update({
                            f'{cam_name}_sample_count': stats['sample_count'],
                            f'{cam_name}_x_mean': stats['x_mean'],
                            f'{cam_name}_x_std': stats['x_std'],
                            f'{cam_name}_y_mean': stats['y_mean'],
                            f'{cam_name}_y_std': stats['y_std'],
                            f'{cam_name}_avg_markers': stats['avg_markers']
                        })
                    else:
                        main_row.update({
                            f'{cam_name}_sample_count': 0,
                            f'{cam_name}_x_mean': None,
                            f'{cam_name}_x_std': None,
                            f'{cam_name}_y_mean': None,
                            f'{cam_name}_y_std': None,
                            f'{cam_name}_avg_markers': None
                        })
                
                main_data.append(main_row)
            
            df_main = pd.DataFrame(main_data)
            filename_main = f'robot_trajectory_detailed_{timestamp}.csv'
            df_main.to_csv(filename_main, index=False)
            self.get_logger().info(f"상세 Trajectory 데이터 저장됨: {filename_main}")
            
        except ImportError:
            self.get_logger().warn("pandas가 설치되지 않아 기본 CSV로 저장합니다.")
            # 기본 CSV 저장 로직...

    # ... 나머지 기존 함수들 (load_transformation_matrix, _conver_tR_to_H 등) ...
