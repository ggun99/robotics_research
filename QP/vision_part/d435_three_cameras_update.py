import numpy as np
import cv2
import cv2.aruco as aruco
import rclpy
from rclpy.node import Node
from sensor_msgs.msg import Image, CameraInfo
from geometry_msgs.msg import PoseStamped, TransformStamped
from std_msgs.msg import Header
from cv_bridge import CvBridge
import time
import tf2_ros
from scipy.spatial.transform import Rotation as R
# from tf_transformations import quaternion_from_matrix

class D435ArucoDetector(Node):
    def __init__(self):
        super().__init__('d435_aruco_detector')
        
        # 로봇 마커들 (0~3번) - 로컬 좌표계
        self.robot_markers_local = {
            0: np.array([-0.045, 0.0565, 0.0]),
            1: np.array([0.045, -0.0565, 0.0]),
            2: np.array([0.045, -0.0565, 0.0]),
            3: np.array([-0.045, 0.0565, 0.0])
        }
    
        self.marker_length = 0.075  # 마커 크기 5cm
        
        # 월드 좌표계 설정 상태
        self.world_established = False
        self.camera_to_world_R = None
        self.camera_to_world_t = None
        
        # 카메라 정보
        self.camera_matrix = None
        self.dist_coeffs = None
        self.camera_info_received = False
        
        # 로봇 중심 추적
        self.latest_robot_center = None
        self.center_timestamp = None
        
        self.aruco_50 = [0, 1,2,3]
        self.aruco_75 = [10]

        # ArUco 설정
        self.aruco_dict = cv2.aruco.getPredefinedDictionary(cv2.aruco.DICT_4X4_50)
        self.aruco_params = cv2.aruco.DetectorParameters()

        # 변환 행렬들
        self.H_world2cam1 = None    # 10번 마커 기준 월드→카메라
        self.H_world2cam2 = None    # 10번 마커 기준 월드→카메라2
        self.H_world2cam3 = None    # 10번 마커 기준 월드→카메라
        self.H_cam2robot = None    # 카메라→로봇 중심
        self.H_world2robot = None  # 월드→로봇
        
        # 월드 기준 카메라 관련
        self.world_reference_camera = None
        self.world_origin_H_world2cam = None
        
        self.tf_broadcaster = tf2_ros.TransformBroadcaster(self)

        self.camera_marker_data = {
            'camera1_color_optical_frame': {},
            'camera2_color_optical_frame': {},
            'camera3_color_optical_frame': {}
        }
        # 구독자 설정
        self.create_subscription(
            Image, '/camera3/camera3/color/image_raw', 
            self.image_callback, 10
        )
        self.create_subscription(
            CameraInfo, '/camera3/camera3/color/camera_info',
            self.camera_info_callback, 10
        )
        self.create_subscription(
            Image, '/camera1/camera1/color/image_raw', 
            self.image_callback, 10
        )
        self.create_subscription(
            CameraInfo, '/camera1/camera1/color/camera_info',
            self.camera_info_callback, 10
        )
        self.create_subscription(
            Image, '/camera2/camera2/color/image_raw', 
            self.image_callback, 10
        )
        self.create_subscription(
            CameraInfo, '/camera2/camera2/color/camera_info',
            self.camera_info_callback, 10
        )
        
        # 발행자
        self.robot_center_pub = self.create_publisher(
            PoseStamped, '/robot_center', 10
        )
        
        self.get_logger().info("D435 ArUco Detector initialized")

    def camera_info_callback(self, msg):
        """카메라 내부 매개변수 수신"""
        if not self.camera_info_received:
            self.camera_matrix = np.array(msg.k).reshape(3, 3)
            self.dist_coeffs = np.array(msg.d)
            self.camera_info_received = True
            self.get_logger().info("Camera intrinsics received")

    def image_callback(self, msg):
        """이미지 처리 및 ArUco 검출"""
        if not self.camera_info_received:
            return
        camera_name = msg.header.frame_id

        bridge = CvBridge()
        cv_image = bridge.imgmsg_to_cv2(msg, desired_encoding='bgr8')
        gray = cv2.cvtColor(cv_image, cv2.COLOR_BGR2GRAY)

        # ArUco 마커 검출
        detector = aruco.ArucoDetector(self.aruco_dict, self.aruco_params)
        corners, ids, _ = detector.detectMarkers(gray)

        if ids is None:
            return

        # 기본 포즈 추정 (카메라 좌표계)
        try:
            rvecs, tvecs, _ = cv2.aruco.estimatePoseSingleMarkers(
                corners, self.marker_length, self.camera_matrix, self.dist_coeffs
            )
        except Exception as e:
            self.get_logger().error(f"Pose estimation failed: {e}")
            return

        # 검출된 마커 정리
        detected_markers = {}
        for i, marker_id in enumerate(ids.flatten()):
            detected_markers[marker_id] = {
                'cam_tvec': tvecs[i].reshape(3),
                'cam_rvec': rvecs[i].reshape(3),
                'corners': corners[i]
            }

        # 🆕 단순한 월드 좌표계 설정: 10번 마커 = 월드 원점
        H_world2cam = None
        robot_markers_cam = {}

        for marker_id, data in detected_markers.items():
            # ✅ 10번 마커를 월드 좌표계로 설정
            if marker_id == 10:
                rvec = data['cam_rvec']
                tvec = data['cam_tvec']
                H_cam2world = np.eye(4)
                R, _ = cv2.Rodrigues(rvec)
                H_cam2world[0:3, 0:3] = R
                H_cam2world[0:3, 3] = tvec
                # 마커 10의 좌표계 = 월드 좌표계
                # H_cam2marker10 의 역변환 = H_marker10→cam = H_world→cam
                H_world2cam = np.linalg.inv(H_cam2world)
                
                # 카메라별로 저장
                if camera_name == 'camera1_color_optical_frame':
                    self.H_world2cam1 = H_world2cam
                elif camera_name == 'camera2_color_optical_frame':
                    self.H_world2cam2 = H_world2cam
                elif camera_name == 'camera3_color_optical_frame':
                    self.H_world2cam3 = H_world2cam
                
                # 월드 좌표계가 아직 설정되지 않았다면 (첫 번째 카메라)
                if not self.world_established:
                    self.broadcast_world_transform(H_world2cam, camera_name, msg.header.stamp)
                    self.world_established = True
                    self.get_logger().info(f"✅ World coordinate system established from {camera_name}")
                else:
                    # 이미 월드가 설정된 경우 (두 번째 카메라)
                    self.broadcast_world_transform(H_world2cam, camera_name, msg.header.stamp)
                    self.get_logger().info(f"📷 {camera_name} position updated relative to world")
                
                self.get_logger().info(f"Marker 10 position in {camera_name}: {tvec}")
            
            # 로봇 마커들 수집
            elif marker_id in self.robot_markers_local:
                rvec = data['cam_rvec']
                tvec = data['cam_tvec']
                R, _ = cv2.Rodrigues(rvec)
                H_cam2marker = np.eye(4)
                H_cam2marker[0:3, 0:3] = R
                H_cam2marker[0:3, 3] = tvec
                robot_markers_cam[marker_id] = {
                    'position': tvec,
                    'rotation': R,
                    'transform': H_cam2marker
                }

        # 10번 마커가 없으면 월드 좌표계 없음
        if not self.world_established:
            self.get_logger().warn("❌ Marker 10 not detected - no world coordinate system")
            self.visualize_results(cv_image, detected_markers, camera_name)
            return

        # 🤖 로봇 중심 계산 (수정됨)
        if len(robot_markers_cam) > 0:
            # 하나의 마커를 기준으로 로봇 중심 계산
            robot_center_cam, robot_rotation_cam = self.calculate_robot_center_from_single_marker(robot_markers_cam)
            
            # H_cam2robot
            H_cam2robot = np.eye(4)
            H_cam2robot[0:3, 0:3] = robot_rotation_cam  # ✅ 직접 사용 (Rodrigues 불필요)
            H_cam2robot[0:3, 3] = robot_center_cam
            self.H_cam2robot = H_cam2robot
                        
            # H_world2robot 계산
            print("H_world2cam:\n", H_world2cam)
            print("H_cam2robot:\n", H_cam2robot)
            H_world2robot = H_world2cam @ H_cam2robot
            robot_center_world = H_world2robot[:3, 3]
            
            self.broadcast_robot_transform(H_world2robot, camera_name, msg.header.stamp)
            self.latest_robot_center = robot_center_world
            self.center_timestamp = time.time()
            
            self.get_logger().info(f"🤖 Robot center (cam): {robot_center_cam}")
            self.get_logger().info(f"🌍 Robot center (world): {robot_center_world}")
            
            # 발행
            self.publish_robot_center(robot_center_world, len(robot_markers_cam))

            # 현재 카메라의 마커 데이터 저장
            self.camera_marker_data[camera_name] = robot_markers_cam

            # 모든 카메라 데이터를 합쳐 로봇 중심 계산
            self.fuse_robot_center_across_cameras()
            
        # 시각화
        self.visualize_results(cv_image, detected_markers, camera_name)

    def project_world_to_image_simple(self, world_point, camera_name):
        """단일 world 기준으로 월드 좌표를 이미지로 투영"""
        
        # world가 설정되지 않았으면 투영 불가
        if self.world_reference_camera is None:
            return None
            
        # 카메라별로 적절한 transform 선택
        if camera_name == 'camera1_color_optical_frame':
            H_world2cam = self.H_world2cam1
        elif camera_name == 'camera2_color_optical_frame':
            H_world2cam = self.H_world2cam2
        elif camera_name == 'camera3_color_optical_frame':
            H_world2cam = self.H_world2cam3
        else:
            return None
            
        if H_world2cam is None:
            return None
            
        # 만약 현재 카메라가 world 기준 카메라가 아니라면, 좌표 변환 필요
        if camera_name != self.world_reference_camera:
            # world → 기준 카메라 → 현재 카메라 변환
            H_world2ref_cam = self.world_origin_H_world2cam
            H_ref_cam2world = np.linalg.inv(H_world2ref_cam)
            H_world2current_cam = H_world2cam
            
            # 실제로는 같은 world이므로 현재 카메라의 H_world2cam 직접 사용
            effective_H_world2cam = H_world2cam
        else:
            effective_H_world2cam = H_world2cam
        
        try:
            # 월드 → 카메라 변환
            world_point_homogeneous = np.append(world_point, 1)
            cam_point_homogeneous = effective_H_world2cam @ world_point_homogeneous
            cam_point = cam_point_homogeneous[:3]
            
            if cam_point[2] <= 0:  # 카메라 뒤에 있음
                return None
            
            # 카메라 → 이미지 투영
            image_points, _ = cv2.projectPoints(
                cam_point.reshape(1, 1, 3), np.zeros(3), np.zeros(3),
                self.camera_matrix, self.dist_coeffs
            )
            
            return image_points[0][0]
            
        except Exception as e:
            self.get_logger().error(f"Projection failed: {e}")
            return None
        
    def fuse_robot_center_across_cameras(self):
        """여러 카메라에서 검출된 마커들을 월드 좌표계로 융합하여 로봇 중심 계산"""
        if not self.world_established:
            self.get_logger().warn("World not established yet, skipping fusion")
            return
        
        robot_markers_world = {}

        # 카메라별 데이터 수집
        camera_transforms = {
            'camera1_color_optical_frame': self.H_world2cam1,
            'camera2_color_optical_frame': self.H_world2cam2,
            'camera3_color_optical_frame': self.H_world2cam3
        }

        for cam_name, cam_data in self.camera_marker_data.items():
            if cam_name not in camera_transforms or camera_transforms[cam_name] is None:
                continue
            
            H_world2cam = camera_transforms[cam_name]
            H_cam2world = np.linalg.inv(H_world2cam)
            
            for mid, marker in cam_data.items():
                if mid not in self.robot_markers_local:
                    continue
                
                # 카메라 좌표 → 월드 좌표
                pos_cam = np.append(marker['position'], 1)
                pos_world = (H_cam2world @ pos_cam)[:3]
                rot_world = H_cam2world[:3, :3] @ marker['rotation']

                robot_markers_world[mid] = {'position': pos_world, 'rotation': rot_world}
        
        if len(robot_markers_world) == 0:
            self.get_logger().warn("No robot markers detected in any camera.")
            return

        # --- 하나의 마커만 사용 (가장 신뢰도 높은 것 선택) ---
        # 여기서는 간단히 ID가 가장 작은 마커를 사용
        best_id = min(robot_markers_world.keys())
        marker_data = robot_markers_world[best_id]

        R_marker_world = marker_data['rotation']
        t_marker_world = marker_data['position']
        offset_local = self.robot_markers_local[best_id]
        offset_world = R_marker_world @ offset_local

        robot_center_world = t_marker_world - offset_world

        # --- tf 및 토픽 발행 ---
        self.latest_robot_center = robot_center_world
        self.broadcast_robot_transform_simple(robot_center_world, R_marker_world)
        self.publish_robot_center(robot_center_world, len(robot_markers_world))

        self.get_logger().info(
            f"🌍 Fused robot_center (world): {robot_center_world} from {len(robot_markers_world)} markers across cameras"
        )


    def broadcast_robot_transform_simple(self, center_world, R_world):
        """로봇 중심을 world 기준으로 브로드캐스트 (하나만 유지)"""
        t = TransformStamped()
        t.header.stamp = self.get_clock().now().to_msg()
        t.header.frame_id = 'world'
        t.child_frame_id = 'robot_center'

        t.transform.translation.x = float(center_world[0])
        t.transform.translation.y = float(center_world[1])
        t.transform.translation.z = float(center_world[2])

        quat = R.from_matrix(R_world).as_quat()
        t.transform.rotation.x = quat[0]
        t.transform.rotation.y = quat[1]
        t.transform.rotation.z = quat[2]
        t.transform.rotation.w = quat[3]

        self.tf_broadcaster.sendTransform(t)


    def publish_robot_center(self, center, num_markers):
        """로봇 중심 위치 발행"""
        msg = PoseStamped()
        msg.header = Header()
        msg.header.stamp = self.get_clock().now().to_msg()
        msg.header.frame_id = 'world'
        
        msg.pose.position.x = float(center[0])
        msg.pose.position.y = float(center[1])
        msg.pose.position.z = float(center[2])
        msg.pose.orientation.w = 1.0
        
        self.robot_center_pub.publish(msg)
        
        self.get_logger().info(
            f"Published robot center: ({center[0]:.3f}, {center[1]:.3f}, {center[2]:.3f}) "
            f"from {num_markers} markers"
        )
        
    def broadcast_robot_transform(self, H_world2robot, camera_name, timestamp):
        """로봇 좌표계 tf 브로드캐스트 - 하나의 world 기준"""
        
        t = TransformStamped()
        t.header.stamp = timestamp
        t.header.frame_id = 'world'  # 모든 로봇은 같은 world를 기준으로
        t.child_frame_id = 'robot_center'
        
        # 위치
        t.transform.translation.x = float(H_world2robot[0, 3])
        t.transform.translation.y = float(H_world2robot[1, 3])
        t.transform.translation.z = float(H_world2robot[2, 3])
        
        # 회전 (H_world2robot을 그대로 사용)
        R_mat = H_world2robot[0:3, 0:3]
        rot = R.from_matrix(R_mat)
        quat = rot.as_quat()  # [x, y, z, w]
        t.transform.rotation.x = quat[0]
        t.transform.rotation.y = quat[1]
        t.transform.rotation.z = quat[2]
        t.transform.rotation.w = quat[3]
        
        self.tf_broadcaster.sendTransform(t)
        
        self.get_logger().info(f"🤖 Broadcasting robot_center from world (detected by {camera_name})")

    def broadcast_world_transform(self, H_world2cam, camera_name, timestamp):
        """월드 좌표계 tf 브로드캐스트 - 모든 카메라를 같은 월드 기준으로 정렬"""
        
        t = TransformStamped()
        t.header.stamp = self.get_clock().now().to_msg()
        t.header.frame_id = "world"            # 부모 프레임
        t.child_frame_id = f"{camera_name}_frame"         # 자식 프레임 (각 카메라)
        
        # H_world2cam은 world → camera 변환 행렬임
        t.transform.translation.x = float(H_world2cam[0, 3])
        t.transform.translation.y = float(H_world2cam[1, 3])
        t.transform.translation.z = float(H_world2cam[2, 3])
        
        R_mat = H_world2cam[0:3, 0:3]
        rot = R.from_matrix(R_mat)
        quat = rot.as_quat()
        t.transform.rotation.x = quat[0]
        t.transform.rotation.y = quat[1]
        t.transform.rotation.z = quat[2]
        t.transform.rotation.w = quat[3]
        
        self.tf_broadcaster.sendTransform(t)
        self.get_logger().info(f"📷 Broadcasting {camera_name} in world frame")
        
        # 첫 번째로 설정된 카메라를 world 기준으로 저장
        if self.world_reference_camera is None:
            self.world_reference_camera = camera_name
            self.world_origin_H_world2cam = H_world2cam
            self.get_logger().info(f"🌍 World reference set by {camera_name}")


    def calculate_robot_center_from_single_marker(self, robot_markers_cam):
        """하나의 마커를 기준으로 로컬 좌표계 오프셋을 이용해 로봇 중심 계산"""
        
        # 가장 신뢰할만한 마커 선택 (가장 작은 ID 또는 가장 가까운 마커)
        best_marker_id = min(robot_markers_cam.keys())  # 가장 작은 ID 선택
        best_marker_data = robot_markers_cam[best_marker_id]
        
        # 선택된 마커의 카메라 좌표계 정보
        marker_position_cam = best_marker_data['position']  # 카메라에서 본 마커 위치
        marker_rotation_cam = best_marker_data['rotation']  # 카메라에서 본 마커 회전
        
        # 로컬 좌표계에서 이 마커가 로봇 중심으로부터 얼마나 떨어져 있는지
        marker_offset_local = self.robot_markers_local[best_marker_id]  # 로컬 좌표계에서의 오프셋
        
        # 마커의 회전 행렬을 이용해 로컬 오프셋을 카메라 좌표계로 변환
        # marker_offset_cam = R_marker_cam @ marker_offset_local
        marker_offset_cam = marker_rotation_cam @ marker_offset_local
        
        # 로봇 중심 = 마커 위치 - 마커 오프셋
        robot_center_cam = marker_position_cam - marker_offset_cam
        
        self.get_logger().info(f"🎯 Using marker {best_marker_id} as reference")
        self.get_logger().info(f"   Marker pos (cam): {marker_position_cam}")
        self.get_logger().info(f"   Marker offset (local): {marker_offset_local}")
        self.get_logger().info(f"   Marker offset (cam): {marker_offset_cam}")
        self.get_logger().info(f"   Robot center (cam): {robot_center_cam}")
        
        return robot_center_cam, marker_rotation_cam

    def calculate_robot_rotation_simple(self, robot_markers_cam):
        """간단한 아웃라이어 제거 + 회전 평균"""
        
        if len(robot_markers_cam) <= 1:
            return list(robot_markers_cam.values())[0]['rotation']
        
        from scipy.spatial.transform import Rotation as R_scipy
        
        # Quaternion 변환
        rotations_data = []
        for marker_id, data in robot_markers_cam.items():
            rot_matrix = data['rotation']
            r = R_scipy.from_matrix(rot_matrix)
            quat = r.as_quat()
            rotations_data.append((marker_id, quat, rot_matrix))
        
        # 간단한 아웃라이어 제거: 첫 번째와 너무 다른 것 제거
        reference_quat = rotations_data[0][1]
        valid_rotations = []
        
        for marker_id, quat, rot_matrix in rotations_data:
            # 각도 차이 계산
            dot_product = np.abs(np.dot(reference_quat, quat))
            angle_diff = 2 * np.arccos(np.clip(dot_product, 0, 1))
            
            if angle_diff <= 0.5:  # 약 30도 이내
                valid_rotations.append(quat)
            else:
                self.get_logger().warn(f"⚠️ Marker {marker_id} removed: angle diff {np.degrees(angle_diff):.1f}°")
        
        # 평균 계산
        if len(valid_rotations) > 0:
            mean_quat = np.mean(valid_rotations, axis=0)
            mean_quat = mean_quat / np.linalg.norm(mean_quat)
            return R_scipy.from_quat(mean_quat).as_matrix()
        else:
            return rotations_data[0][2]  # 첫 번째 마커 사용
    
    def visualize_results(self, image, detected_markers, camera_name):
        """검출 결과와 좌표계 축 시각화"""
        
        # 검출된 마커들 표시
        for marker_id, data in detected_markers.items():
            corners = data['corners'][0]
            
            
            
            # 마커 ID 표시 - 색상 구분
            center = np.mean(corners, axis=0).astype(int)
            
            if marker_id == 10:
                # 10번 마커 - 월드 원점 (파란색)
                color = (255, 0, 0)
                label = f"World{marker_id}"
            # elif marker_id in self.robot_markers_local:
            #     # 로봇 마커들 (빨간색)
            #     color = (0, 0, 255)
            #     label = f"R{marker_id}"
                # 마커 경계 그리기
                cv2.polylines(image, [corners.astype(int)], True, (0, 255, 0), 2)
                cv2.putText(image, label, tuple(center), 
                        cv2.FONT_HERSHEY_SIMPLEX, 1, color, 2)
            
                # # 🆕 마커별 좌표축 그리기
                self.draw_coordinate_axes(image, data['cam_rvec'], data['cam_tvec'], 0.05)

        # 🤖 로봇 중심과 로봇 좌표축 표시
        if (self.world_established and 
            self.latest_robot_center is not None and
            self.center_timestamp is not None and 
            time.time() - self.center_timestamp < 1.0):
            
            # 로봇 중심을 이미지로 투영
            center_image = self.project_world_to_image_simple(self.latest_robot_center, camera_name)
            if center_image is not None:
                center_pt = tuple(center_image.astype(int))
                
                # 로봇 중심 표시
                cv2.circle(image, center_pt, 8, (0, 255, 255), -1)
                cv2.putText(image, "Robot", (center_pt[0]+15, center_pt[1]), 
                        cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 2)
                
                # 🆕 로봇 좌표축 그리기
                self.draw_robot_coordinate_axes(image)

        # 📊 간단한 상태 정보만
        world_status = "World: OK" if self.world_established else "World: No Marker 10"
        world_color = (0, 255, 0) if self.world_established else (0, 0, 255)
        cv2.putText(image, world_status, (10, 30), 
                cv2.FONT_HERSHEY_SIMPLEX, 0.7, world_color, 2)
        
        # 로봇 좌표 (간단히)
        if self.latest_robot_center is not None:
            coord_text = f"Robot: ({self.latest_robot_center[0]:.2f}, {self.latest_robot_center[1]:.2f}, {self.latest_robot_center[2]:.2f})"
            cv2.putText(image, coord_text, (10, 60), 
                    cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 2)

        # 이미지 표시
        cv2.imshow(f'ArUco Detection with Coordinate Axes in {camera_name}', image)
        cv2.waitKey(1)

    def draw_robot_coordinate_axes(self, image):
        """로봇 좌표축을 draw_coordinate_axes 함수를 사용해서 그리기"""
        
        if self.H_cam2robot is None:
            return
        
        try:
            # H_cam2robot에서 rvec, tvec 추출
            robot_rotation = self.H_cam2robot[0:3, 0:3]  # 회전 행렬
            robot_translation = self.H_cam2robot[0:3, 3]  # 이동 벡터
            
            # 회전 행렬 → rodrigues 벡터 변환
            robot_rvec, _ = cv2.Rodrigues(robot_rotation)
            robot_tvec = robot_translation
            
            # ✅ 기존 draw_coordinate_axes 함수 사용
            self.draw_coordinate_axes(image, robot_rvec, robot_tvec, length=0.08)
            
            self.get_logger().info("✅ Robot coordinate axes drawn successfully")
            
        except Exception as e:
            self.get_logger().warn(f"❌ Failed to draw robot coordinate axes: {e}")
            
    def draw_coordinate_axes(self, image, rvec, tvec, length=0.03):
        """마커의 좌표축을 이미지에 그리기"""
        try:
            # 축 포인트들 정의 (마커 중심에서 각 축 방향으로)
            axis_points = np.float32([[0,0,0], [length,0,0], [0,length,0], [0,0,length]]).reshape(-1,3)
            
            # 3D 점들을 이미지로 투영
            imgpts, _ = cv2.projectPoints(axis_points, rvec, tvec, 
                                        self.camera_matrix, self.dist_coeffs)
            
            imgpts = np.int32(imgpts).reshape(-1,2)
            
            # 원점
            origin = tuple(imgpts[0])
            
            # X축 (빨간색)
            cv2.arrowedLine(image, origin, tuple(imgpts[1]), (0,0,255), 2)
            # Y축 (초록색)  
            cv2.arrowedLine(image, origin, tuple(imgpts[2]), (0,255,0), 2)
            # Z축 (파란색)
            cv2.arrowedLine(image, origin, tuple(imgpts[3]), (255,0,0), 2)
            
        except Exception as e:
            pass  # 조용히 무시

def main(args=None):
    rclpy.init(args=args)
    detector = D435ArucoDetector()
    
    try:
        rclpy.spin(detector)
    except KeyboardInterrupt:
        pass
    finally:
        cv2.destroyAllWindows()
        detector.destroy_node()
        rclpy.shutdown()

if __name__ == '__main__':
    main()