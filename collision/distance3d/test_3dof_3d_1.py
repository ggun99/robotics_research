...
def Link_Joint(theta, phi, gamma):
    center1 = np.array([0,0,L1_len])
    #Link1
    ...
    return L1, L2, L3, J1, J2


def collision_check(data_, data):
    ...
    for i in range(data_.shape[0]):
        ...
        L1, L2, L3, J1, J2 = Link_Joint(theta, phi, gamma)
        ...
        if result[i] == 1 : 
            collision_state = 1
            dataset.append([s,d,f,collision_state])
        elif result[i] == 0 :
            collision_state = -1
            dataset.append([s,d,f,collision_state])         
    # end = time.time()
...
if __name__ == '__main__':
    ...
    data_, data = data_generation()
    print('data generation finish')
    print('collision check start')
    collision_check(data_, data)
    print('collision check finish')
    c_svm()